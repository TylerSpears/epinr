# -*- coding: utf-8 -*-
# Module for functions and classes related to frequency-based features.
import torch
...
def gaussian_kernel_3d(kernel_radius: int, sigma: float) -> torch.Tensor:
    ...
    k = (1 / 2 * torch.pi * sigma**2) * torch.exp(
...
class _LPTNResidualBlock(torch.nn.Module):
...
class _LPTNLowFreqTranslateNet(torch.nn.Module):
    def __init__(self, num_channels=6, num_residual_blocks=5):
        ...
        for _ in range(num_residual_blocks):
            self.res_blocks.append(_LPTNResidualBlock(64))
...
class _LPTNHighFreqTranslateNet(torch.nn.Module):
    def __init__(self, num_input_channels, num_residual_blocks=3):
        ...
        self.res_blocks = torch.nn.ModuleList()
        for _ in range(num_residual_blocks):
            self.res_blocks.append(_LPTNResidualBlock(16))
...
class LPTN(torch.nn.Module):
    def __init__(self, num_input_channels, num_high_freq_levels=3):
        """3D Laplacian Pyramid Translation Network implementation.

        From:
        J. Liang, H. Zeng, and L. Zhang, “High-Resolution Photorealistic Image
            ...
            and Pattern Recognition, 2021, pp. 9392-9400. Accessed: Aug. 26, 2021.
            [Online]. Available:
            https://openaccess.thecvf.com/content/CVPR2021/html/Liang_High-Resolution_Photorealistic_Image_Translation_in_Real-Time_A_Laplacian_Pyramid_Translation_CVPR_2021_paper.html
        ...
        self.low_freq_net = _LPTNLowFreqTranslateNet(
        ...
        high_freq_nets.append(
            _LPTNHighFreqTranslateNet(
                self.num_input_channels * 3, num_residual_blocks=3
            )
        )
        for _ in range(num_high_freq_levels - 1):
            high_freq_nets.append(_LPTNHighFreqTranslateNet(1, num_residual_blocks=0))
