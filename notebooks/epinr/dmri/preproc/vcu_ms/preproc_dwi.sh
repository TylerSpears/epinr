...
echo "Slice timing file: $slspec_f"
...
slspec_ext="${slspec_f##*.}"
if [ "$slspec_ext" == "txt" ]; then
    echo "Using FSL-style slice specification file: $slspec_f"
    slspec_arg="--slspec=${slspec_f}"
elif [ "$slspec_ext" == "json" ]; then
    echo "Using JSON sidecar slice specification file: $slspec_f"
    # Clean and get the absolute path to the json sidecar.
    tmp_json_sidecar="${tmp_dir}/json_sidecar.json"
    /usr/bin/cat "$slspec_f" | sed 's/\\//g' | jq -Mc '.' >"$tmp_json_sidecar"
    json_sidecar="$(realpath "$tmp_json_sidecar")"
    slspec_arg="--json=${json_sidecar}"
else
    echo "Unknown slice specification file extension: $slspec_ext"
    return 1
fi
