...
def test_transform_3d_coords_simple_affines():
    # Unbatched
    s = 10
    coords = torch.stack(
        torch.meshgrid(
    ...
    # Identity
    c_t = mrinr.coords.transform_coords(coords, affine)
    ...
    a = affine.clone()
    a[:3, :3] = a[:3, :3] / 2
    c_t = mrinr.coords.transform_coords(coords, a)
...
def test_transform_3d_coords_vs_pytransform3d_outputs():
    p_a = np.stack(
        [
            [0, 0, 0],
            [1, 1, 1],
    ...
    for t in translations:
        ...
        for r in rotations:
            ...
            mrinr_p_b = mrinr.coords.transform_coords(torch_p_a, torch_A)
            assert torch.isclose(torch.from_numpy(p3d_p_b), mrinr_p_b).all()


def test_transform_3d_coords_batch_and_position_invariance():
    # b = 3
    s = 10
    coords = torch.stack(
        torch.meshgrid(
...
def test_canonicalize_coords_3d_affine_valid_inputs():
    ...
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(coords_b, affine_b)
    assert tuple(c_res.shape) == (b, s, s, s, 3)
    assert tuple(a_res.shape) == (b, 4, 4)
    assert c_res.dtype == a_res.dtype

    # Only coords is batched.
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(coords_b, affine)
    assert tuple(c_res.shape) == (b, s, s, s, 3)
    assert tuple(a_res.shape) == (b, 4, 4)

    # Only affine is batched.
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(coords, affine_b)
    assert tuple(c_res.shape) == (b, s, s, s, 3)
    assert tuple(a_res.shape) == (b, 4, 4)

    # Neither is batched, so batch size is 1.
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(coords, affine)
    ...
    # 1D
    c = coords_b[0, 0, 0, 0]
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(c, affine_b)
    assert tuple(c_res.shape) == (b, 1, 1, 1, 3)
    assert tuple(a_res.shape) == (b, 4, 4)
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(c, affine)
    ...
    # 4D
    # Ambiguous case, canonicalize assumes that the batch size is 1 and the first
    ...
    c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(c, a)
...
def test_canonicalize_coords_3d_affine_invalid_inputs():
    ...
    a = affine_b[:-4]
    with pytest.raises(RuntimeError):
        c_res, a_res = mrinr.coords._canonicalize_coords_3d_affine(coords_b, a)
...
def test_canonicalize_coords_2d_affine_valid_inputs():
    # Both batched, same batch size.
    b = 5
    affine = torch.eye(3)
    affine_b = einops.repeat(affine, "a_1 a_2 -> b a_1 a_2", b=b)
    ...
    coords_b = einops.repeat(coords, "s1 s2 coord -> b s1 s2 coord", b=b)

    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(coords_b, affine_b)
    assert tuple(c_res.shape) == (b, s, s, 2)
    assert tuple(a_res.shape) == (b, 3, 3)
    assert c_res.dtype == a_res.dtype

    # Only coords is batched.
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(coords_b, affine)
    assert tuple(c_res.shape) == (b, s, s, 2)
    assert tuple(a_res.shape) == (b, 3, 3)

    # Only affine is batched.
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(coords, affine_b)
    assert tuple(c_res.shape) == (b, s, s, 2)
    assert tuple(a_res.shape) == (b, 3, 3)

    # Neither is batched, so batch size is 1.
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(coords, affine)
    assert tuple(c_res.shape) == (1, s, s, 2)
    assert tuple(a_res.shape) == (1, 3, 3)

    # Variants on the coordinates shape.
    # 1D
    c = coords_b[0, 0, 0]
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(c, affine_b)
    assert tuple(c_res.shape) == (b, 1, 1, 2)
    assert tuple(a_res.shape) == (b, 3, 3)
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(c, affine)
    assert tuple(c_res.shape) == (1, 1, 1, 2)
    assert tuple(a_res.shape) == (1, 3, 3)
    ...
    c = coords_b.to(torch.float32)
    a = affine_b.to(torch.float64)
    c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(c, a)
    assert c_res.dtype == torch.float64
    assert a_res.dtype == torch.float64


def test_canonicalize_coords_2d_affine_invalid_inputs():
    # Both batched, different batch sizes.
    b = 3
    affine = torch.eye(3)
    affine_b = einops.repeat(affine, "a_1 a_2 -> b a_1 a_2", b=b)
    ...
    a = affine_b[:-4]
    with pytest.raises(RuntimeError):
        c_res, a_res = mrinr.coords._canonicalize_coords_2d_affine(coords_b, a)
...
#             sample_mrinr = mrinr.vols.sample_vol(
#                 self.torch_grid_a,
...
#             assert torch.isclose(sample_scipy.flatten(), sample_mrinr.flatten()).all()
...
#                     sample_coords = mrinr.coords.transform_coords(
...
#                     vol_tf_mrinr = mrinr.vols.sample_vol(
#                         input_grid_image,
#                         sample_coords,
...
#                     assert tuple(vol_tf_mrinr.shape) == (
#                         sample_coords.shape[0],
#                     ) + tuple(sample_coords.shape[1:-1])

#                     assert torch.isclose(vol_tf_scipy, vol_tf_mrinr).all()
